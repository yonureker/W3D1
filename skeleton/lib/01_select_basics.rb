...
def per_capita_gdp
  ...
  execute(<<-SQL)
    select name, (gdp/population) as per_capita_gdp from countries where area > 5000000 
  SQL
end
...
def just_the_right_size
  ...
  execute(<<-SQL)
    select name, (area / 1000) AS area_1000s from countries WHERE area BETWEEN 200000 AND 250000
  SQL
end
